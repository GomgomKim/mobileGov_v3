package kr.go.mobile.iff.sample.push;
...
public class LocalPushUtils {
    ...
    public static int register(Context context, String serverAddress, String appID, String userId, PushAppRegistListener pushAppRegistListener) {
        int ret = PushLibrary.getInstance().setStart(context, serverAddress, appID);
        if (ret !=  1400) {
            return ret;
        }
        return PushLibrary.getInstance().AppRegist(context, pushAppRegistListener, userId,
                null, null);
    }
