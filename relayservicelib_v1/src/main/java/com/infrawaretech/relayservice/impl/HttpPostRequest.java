...
public class HttpPostRequest extends HttpRequest {
	...
	@Deprecated
	public HttpPostRequest(String serviceId) throws MalformedURLException {
		super(serviceId);
	}
	...
	public Map<String, String> generateHeader() {
		...
		if(getContentType() == null) {
			headers.put(HTTPHeader.CONTENT_TYPE, HTTPHeader.CONTENT_POST_VALUE);
		}
		else {
			headers.put(HTTPHeader.CONTENT_TYPE, getContentType());
		}
		
		if(getBodyString() != null){
			headers.put(HTTPHeader.CONTENT_LENGTH, getBodyString().getBytes().length+"");
		}
